...
**「Load recommended settings」ボタン（2026-09-16〜）**: ResRefine アコーディオンの formula ドロップダウン直上に置かれた `gr.Button`（`elem_id="resrefine-load-recommended"`）。以下のいずれもクリック時（`btn.click`）にのみ発生し、生成時に隠れた解決は一切行われない。

- **捕獲経路**: 本体（Forge Neo core）の `{tabname}_sampling` / `{tabname}_scheduler` / `{tabname}_steps` コンポーネント（`tabname` = `txt2img`/`img2img`）を、モジュール import 時にグローバル登録される `hareskip.callbacks.on_after_component`（`script_callbacks.on_after_component`、ControlNet の `on_after_component` + elem_id フィルタ方式を踏襲）が `CORE_COMPONENTS`（elem_id をキーにした dict）へ捕獲する。per-Script の遅延登録では、拡張の `ui()` 構築時点で本体サンプラー欄が既に生成済みのため取り逃すので、import 時のグローバル登録が必須。
- **押下時解決**: `resrefine_load_recommended.click(fn=_hareskip_apply_recommended_ui, inputs=[CORE_COMPONENTS[...]の該当タブ3キー], outputs=[formula, strength, slope_ema, use_after, apply_from_skip, taylor2_curve_strength(interactiveのみ), curve_ema(interactiveのみ), status])`。値計算は gradio 非依存の `_resolve_recommendation(sampler, scheduler, steps)`（`hareskip/recommended_settings.py` の `lookup_recommendation` を引く）が担い、`_hareskip_apply_recommended_ui` がそれを gradio の `gr.update` に変換するだけの薄い皮。interactivity の判定は既存の `_hareskip_prediction_control_updates` を実際に呼び出して合成する（ロジックの重複コピーを避けるため）。
- **生成時の隠れた解決は無い**: ボタンは `script_args` に入らず（UI 引数数は 39 のまま不変）、`STATE` も変えない。書き込むのは formula ドロップダウンとスライダーの**表示値そのもの**であり、生成パスはそれらの表示値を通常どおり読むだけ。ボタンに一度も触れなければ挙動は完全に不変。
- **該当なし → Reuse。他の値は不変**: `lookup_recommendation` が `None` を返した場合（サンプラー・スケジューラの完全一致なし。例: `Euler a` は `Euler` 行にマッチしない）、formula は `Reuse (residual only)` に切り替わるが、`prediction_strength` / `slope_ema_smoothing` / `use_prediction_after_progress` / `apply_prediction_from_skip` の値は**変更しない**（Reuse では使われない値であり、ユーザーが実験中の値を破壊しないため）。status Markdown に「No calibrated entry for **<sampler> + <scheduler>** — applied Reuse (residual only), the safe default.」を表示する。
- **フォールバック無効化の挙動**: 拡張の `ui()` 構築時点で、当該タブの `{tabname}_sampling`/`_scheduler`/`_steps` の3キーが `CORE_COMPONENTS` に揃っていない場合（Settings → User interface → Parameter order で `scripts` が `sampler` より前に並べ替えられている等）、ボタンは `interactive=False` で無効化され、status に代替の案内文（Parameter order の確認を促す文言）を表示する。txt2img/img2img は別タブとして個別に判定し、片方のタブ構築時点でもう片方の3キーが未捕獲であることは正常であり待たない。捕獲レジストリは Reload UI 時に `on_before_reload` フックでリセットされ、再構築後も正しく動作する。
- テーブル本体（サンプラー×スケジューラごとの推奨値、較正出所、行の追加方法、既知の限界）は `docs/RECOMMENDED-SETTINGS.md` / `docs/RECOMMENDED-SETTINGS.json` を正本ドキュメントとする（数値の正本は `hareskip/recommended_settings.py`）。
